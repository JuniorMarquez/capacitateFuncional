...
$scope.guardarInscripcion = function (item) {

    
    var inscripcion = [];

inscripcion.status='pendiente';
    inscripcion.tituloCurso=item.titulo;
    inscripcion.inversionCurso=item.inversion;
    inscripcion.nombres=item.nombres;
    inscripcion.apellidos=item.apellidos;
    inscripcion.nacionalidad=item.nacionalidad;
    inscripcion.identificacion=item.identificacion;
    inscripcion.nivelInstruccion=item.nivelInstruccion;
    inscripcion.profesion=item.profesion;
    inscripcion.sector=item.sector;
    inscripcion.institucionEmpresa=item.institucionEmpresa;
    inscripcion.email=item.email;
    inscripcion.telefono=item.telefono;
    inscripcion.direccion=item.direccion;
    inscripcion.referencia=item.referencia;
    inscripcion.banco=item.banco;



    $http.post('http://52.39.15.75:1346/inscripcion/', inscripcion)
    $modalInstance.close();
$scope.popInscripcion();
  ...
  $http.get('http://52.39.15.75:1345/electoral').then(function (resp) {
    $scope.listadoElectoral = resp.data.results;
  });
    $http.get('http://52.39.15.75:1345/deporte').then(function (resp) {
...
{nombre:'Extranjero',valor:'E'}
   ...
   $scope.niveles=[
{nombre:'Bachiller',valor:'b'},
{nombre:'Técnico',valor:'t'},
{nombre:'Profesional Universitario',valor:'p'}
  ]
   $scope.sectores=[
{nombre:'Público',valor:'pu'},
{nombre:'Privado',valor:'pr'},
{nombre:'Servicios Profesionales',valor:'sp'}
  ]
...
$scope.consultaEvento=function(item){
  ...
  $scope.datosMiembro={};
  $http.get('http://52.39.15.75:1345/evento/'+identificador).success(function(respuesta){        
    item=respuesta;
...
$scope.consultaMiembro=function(item){
  ...
  $scope.datosMiembro={};
  $http.get('http://52.39.15.75:1345/miembro/'+identificador).success(function(respuesta){        
    item=respuesta;
    $scope.item=item;
  });
  item=$scope.item;
...
$scope.consultaCapacitacionInscripcion=function(item){
  var item=[];
  var identificador = MyService.data.idenCapacitacionInscripcion;
  $scope.datosCapacitacion={};
  ...
  $scope.item=item;
    ...
    $http.post('http://52.39.15.75:1345/miembro/', item)
    $modalInstance.close();
    $state.go('access.ok');
};
$scope.okMision= function (item) {
  var configAct = {};
      ...
      $http.put('http://52.39.15.75:1345/configuracion/'+MyService.data.idConfig , configAct);       
      $modalInstance.close();
    };
...
$scope.borrar=function(item){
   var idRaza=item.id;
      $http.delete('http://52.39.15.75:1345/raza/'+idRaza , item)
      $modalInstance.dismiss('cancel');
};

$scope.borrarEspecialidad=function(item){
   var idEspecialidad=item.id;
      $http.delete('http://52.39.15.75:1345/especialidad/'+idEspecialidad , item)
      $modalInstance.dismiss('cancel');
};

$scope.borrarDeporte=function(item){
   var idDeporte=item.id;
      $http.delete('http://52.39.15.75:1345/deporte/'+idDeporte , item)
      $modalInstance.dismiss('cancel');
};
$scope.borrarElectoral=function(item){
   var idElectoral=item.id;
      $http.delete('http://52.39.15.75:1345/electoral/'+idElectoral , item)
      $modalInstance.dismiss('cancel');
};

$scope.borrarDirectiva=function(item){
   var idDirectiva=item.id;
      $http.delete('http://52.39.15.75:1345/directiva/'+idDirectiva , item)
      $modalInstance.dismiss('cancel');
};
$scope.borrarComite=function(item){
   var idComite=item.id;
      $http.delete('http://52.39.15.75:1345/comite/'+idComite , item)
      $modalInstance.dismiss('cancel');
};

$scope.borrarUniversidad=function(item){
   var idUniversidad=item.id;
      $http.delete('http://52.39.15.75:1345/universidad/'+idUniversidad , item)
      $modalInstance.dismiss('cancel');
};
$scope.borrarMunicipio=function(item){
   var idMunicipio=item.id;
      $http.delete('http://52.39.15.75:1345/municipio/'+idMunicipio , item)
      $modalInstance.dismiss('cancel');
};
   ...
   $scope.filterOptions = {
    ...
    $scope.getPagedDataAsync = function (pageSize, page, searchText) {
        setTimeout(function () {
           var data ;
          $http.get('http://52.39.15.75:1345/miembro/').then(function (resp2) {
    ...
    $scope.okNuevaSolicitud = function (item) {
      ...
      $http.post('http://52.39.15.75:1345/solicitud/' ,item);  
      $modalInstance.close();
...
$scope.okEspecialidad = function (item) {
      ...
      $http.post('http://52.39.15.75:1345/especialidad/' ,item);       
      $modalInstance.close();
    };
    $scope.okDirectiva = function (item) {

      item.idUsuario=MyService.data.idUsuario;
      $http.post('http://52.39.15.75:1345/directiva/' ,item);       
      $modalInstance.close();
    };
    $scope.okComite = function (item) {

      item.idUsuario=MyService.data.idUsuario;
      $http.post('http://52.39.15.75:1345/comite/' ,item);       
      $modalInstance.close();
    };
    $scope.okDeporte = function (item) {

      item.idUsuario=MyService.data.idUsuario;
      $http.post('http://52.39.15.75:1345/deporte/' ,item);       
      $modalInstance.close();
    };
    $scope.okElectoral = function (item) {

      item.idUsuario=MyService.data.idUsuario;
      $http.post('http://52.39.15.75:1345/electoral/' ,item);       
      $modalInstance.close();
    };
...
$scope.okAprobacion = function (item) {
    ...
    $http.put('http://52.39.15.75:1346/suscripcion/'+identificador, suscripcionAct);
    $modalInstance.close();
    setTimeout(function() { 
      $http.get('http://52.39.15.75:1346/suscripcion/' ).success(function(respuesta){
 ...
 $scope.okEdicion = function (item) {
    ...
    $http.put('http://52.39.15.75:1345/miembro/'+identificador, miembroAct);
    $modalInstance.close();
    setTimeout(function() { 
...
$scope.okUniversidad= function (item) {
      item.idUsuario=MyService.data.idUsuario;
      $http.post('http://52.39.15.75:1345/universidad/' ,item);       
      $modalInstance.close();
    };
    $scope.okMunicipio= function (item) {

      item.idUsuario=MyService.data.idUsuario;
      $http.post('http://52.39.15.75:1345/municipio/' ,item);       
      $modalInstance.close();
    };
...
$scope.okActividad = function (item) {
      ...
      $http.post('http://52.39.15.75:1345/evento/' ,item);
    ...
    $scope.okEditarActividad = function (item) {
      ...
      item.idUsuario=MyService.data.idUsuario;
      item.tipo=MyService.data.tipoActividad;
      $http.put('http://52.39.15.75:1345/evento/'+item.id ,item);       
      $modalInstance.close();
   $scope.popEditEvento();
     setTimeout(function() {
        $state.go('app.dashboard-v1');
 ...
 $scope.okMensaje = function (item) {
      ...
      $http.post('http://52.39.15.75:1345/mensaje/' ,item);       
      $modalInstance.close();
  };
    $scope.toaster = {
    title: 'Exito',
    ...
    typeIn: 'success',
    textIn: 'Inscripcion procesada con éxito',
    titleIn: 'Cuidado',
 ...
 $scope.popInscripcion = function(){
    toaster.pop($scope.toaster.typeIn, $scope.toaster.titleIn, $scope.toaster.textIn);
  };
  ...
  $scope.popBorrado = function(){
    ...
    $scope.okBorrarMiembro = function (item,timeout) { 
      ...
      $http.delete('http://52.39.15.75:1345/miembro/'+dato); 
      $scope.popBorrado();
      $modalInstance.close();
      setTimeout(function() {
        $state.go('app.dashboard-v1');
    ...
    //   $http.delete('http://52.39.15.75:1345/articulo/'+idArticulo , item)
    //   $scope.items = null;
    //   $scope.item = null;
    ...
    $scope.okConfirmProfesional = function (item) { 
      ...
      $http.delete('http://52.39.15.75:1345/miembro/'+idProfesional , item)
      $scope.items = null;
      $scope.item = null; 
      $modalInstance.close();
    };
...
$scope.item.datosCuenta=datosCuenta;
  ...
  ; 
  app.controller('TabsDemoCtrl', ['$scope','$http', function($scope,$http) {
     $http.get('http://52.39.15.75:1346/areaConocimiento').then(function (resp) {
    $scope.tabs = resp.data.results;
  });
    // $scope.tabs = [
  ...
  app.controller('TypeaheadDemoCtrl', ['$scope', '$http','MyService','$state', '$filter', 'filterFilter',function($scope, $http, MyService, $state,$filter,filterFilter) {
    ...
    $http.get('http://52.39.15.75:1345/articulo/?idUsuario='+MyService.data.idUsuario).then(function (resp) {
